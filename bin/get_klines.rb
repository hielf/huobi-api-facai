...
HuobiApi.configure do |config|
  config.proxy = 'malongshuai.cn:8118'
  config.access_key = 'ff974e67-ez2xc4vb6n-b6d764e6-b589d'
  config.secret_key = 'a62b57ce-180b0090-90f1d29a-9a664'
      ...
      Async do |task|
        ...
        DB.open(symbol, type) do |db|
          klines[key].each do |start_epoch, data|
            ...
            # 保存当前所有已保存的最新key
            if start_epoch > db["max_epoch"].to_i
              db["max_epoch"] = start_epoch.to_s
              db['keys'] = JSON.parse(db['keys'] || []).push(start_epoch.to_s)
            end
